...
def ReceiveUntil(Socket, ExpectedResponse):
    while True:
        ...
        if Message.startswith("INCOMING "):
            ...
            if len(Parts) != 4:
                raise Exception(f"Malformed INCOMING message: {Message}")
            Sender = Parts[1]
            FileName = Parts[2]
            try:
                FileSize = int(Parts[3])
            except ValueError:
                raise Exception(f"Invalid file size: {Parts[3]}")
            ReceiveFileData(Socket, FileSize)
            print(f"    Incoming file: {FileName} "f"from {Sender} ({FileSize} bytes)")
            continue
...
def TestClient(ClientNumber):
    ...
    try:
        ...
        SendCommand(Socket,f"MSG {TargetUser} Hello from {UserName}")

        ReceiveUntil(Socket,f"OK MSG {TargetUser}")
        EndTime = time.perf_counter()
        print(f"[{UserName}] MSG successful "f"({(EndTime - StartTime) * 1000:.2f} ms)")
        ...
        StartTime = time.perf_counter()
        SendCommand(Socket,f"SEND {TargetUser} test.txt {FileSize}")
        SendFileData(Socket, FILE_PATH)
        ReceiveUntil(Socket,f"OK SEND {TargetUser} test.txt")
        EndTime = time.perf_counter()
        print(f"[{UserName}] SEND successful "f"({(EndTime - StartTime) * 1000:.2f} ms)")
        # Record success
        with ResultsLock:
            Results.append(True)
...
def Main():
    ...
    for ClientNumber in range(1, NUM_CLIENTS + 1):
        Thread = threading.Thread(target=TestClient,args=(ClientNumber,))
